...
namespace XG.Business.Helper
{
	public class Rrd
	{
		public RrdDb GetDb ()
		{
			...
			// Never let RrdDb find out that the file is missing: its constructor throws after the
			// object exists, and that object's finalizer then crashes the whole process (first start).
			if (!System.IO.File.Exists(_dbPath))
			{
				CreateDb(_dbPath, heartBeat);
			}
		...
		static void CreateDb(string _dbPath, int heartBeat)
		{
