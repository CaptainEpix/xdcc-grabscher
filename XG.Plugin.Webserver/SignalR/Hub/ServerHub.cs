...
namespace XG.Plugin.Webserver.SignalR.Hub
{
	public class ServerHub : AObjectHub
	{
        ...
        {
                string serverString = aName.Trim();

                bool useSsl = serverString.StartsWith(
                        "ircs://",
                        StringComparison.OrdinalIgnoreCase);

                if (useSsl)
                {
                        serverString = serverString.Substring(7);
                }

                int port = useSsl ? 6697 : 6667;

                int portSeparator = serverString.LastIndexOf(':');
                if (portSeparator > 0 && portSeparator < serverString.Length - 1)
                {
                        int parsedPort;
                        if (int.TryParse(serverString.Substring(portSeparator + 1), out parsedPort))
                        {
                                port = parsedPort;
                                serverString = serverString.Substring(0, portSeparator);
                        }
                }

                if (useSsl)
                {
                        serverString = "ircs://" + serverString;
                }
